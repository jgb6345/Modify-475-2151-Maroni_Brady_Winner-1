...
foreach ($n in $args){
	...
	elseif ($n -like '*--searchtype==*'){
		$n = $n.Substring(14)
		if (($n -lt 9) -and ($n -ge 0)){$SearchType = $n} #Just Create Date Compare
...
if($mode -eq "csv"){
	...
	elseif($SearchType -eq 8){
		Get-ChildItem $DirPath -Recurse | where-object{$_.GetAccessControl().Owner -like $user} | Sort-Object -Property lastwritetime -Descending | select-object FullName, LastWriteTime, LastAccessTime, CreationTime, Length, @{N='Owner';E={$_.GetAccessControl().Owner}}, @{N='Access';E={$_.GetAccessControl().Accesstostring}} | out-file -filepath $OutFile -force
	}
}
